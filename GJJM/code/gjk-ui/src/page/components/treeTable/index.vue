<template>
  ...
  >
    <!--  -->
    <el-table-column width="25"  align="center"></el-table-column>
    ...
    >
      <template slot-scope="scope">
        <!-- Todo -->
        <!-- eslint-disable-next-line vue/no-confusing-v-for-v-if  v-if="index === 0" -->
        <span v-for="space in scope.row._level" :key="space">
          <span  v-if="index === 0" class="ms-tree-space">{{index}}</span>
        </span>
        <!-- <span v-for="space in scope.row._level"   :key="space" class="ms-tree-space"/> -->
        <!-- <span
