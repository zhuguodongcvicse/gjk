        ...
        <div class="grid-content">
          ...
          <div class="div1">
            ...
            <div class="dispose_form1">
              <el-form :model="dynamicValidateForms" ref="dynamicValidateForm" label-width="82px">
                <el-form-item
                  v-for="(domain, index) in dynamicValidateForms.yuliang.xmlEntityMaps"
                  :label="domain.lableName"
                  :key="domain.key"
                  :prop="'domains.' + index + '.value'"
                >
                  <el-input v-model="domain.attributeMap.name"></el-input>
                </el-form-item>
              </el-form>
            </div>
          </div>
              ...
              <el-form :inline="true" class="demo-form-inline">
                <el-form-item label="是否进行软硬件映射：">
                  ...
                  <el-radio-group v-model="whether" size="medium">
                    <el-radio v-model="radio" label="Y"></el-radio>
                    <el-radio v-model="radio" label="N"></el-radio>
                  </el-radio-group>
                </el-form-item>
              </el-form>
            </div>
...
export default {
  ...
  created() {
    ...
    getSysConfigXmlEntityMap(this.$route.query.proId).then(Response => {
      this.xmlEntitysMaps = Response.data.data;
      console.log("bbbb:::", Response.data.data);
      console.log("aaaa:::", Response.data.data.xmlEntityMaps);
      //往对象为ffff中，设置属性为ooooo[j]的值为Response.data.data.xmlEntityMaps[i].xmlEntityMaps[p].attributeMap.name
      this.$set(
        this.dynamicValidateForms,
        "yuliang",
        Response.data.data.xmlEntityMaps[2]
      );
      console.log("pppp:::", this.dynamicValidateForms);
    });
    ...
    console.log("wwww:::", this.$route.query.proId);
    rollbackDispose(this.$route.query.proId).then(val => {
      ...
      //内存空间的回显
      val.data.data.xmlEntitys[4].xmlEntitys[3].xmlEntitys.forEach(element => {
      ...
      for (let item of val.data.data.xmlEntitys[4].xmlEntitys) {
        ...
        // console.log("indexsss::::", this.indexs);
        if (this.indexs > 6) {
          length = this.indexs - 6;
          this.arrays = val.data.data.xmlEntitys[4].xmlEntitys.slice(-length);
        }
      }
      // 对取到的特殊处理数组分析
      for (var i = 0; i < length; i++) {
        ...
        this.res[0].xmlEntitys[0].attributeName = this.arrays[i].attributeName;
        this.res[0].xmlEntitys[0].attributeNameValue = JSON.parse(
          JSON.stringify(arr)
        );
        this.handlerForm.domains.push(JSON.parse(JSON.stringify(this.res[0])));
  ...
  methods: {
    ...
    handleSavePro() {
      ...
      //给起始构件赋值
      this.entity.xmlEntitys[4].xmlEntitys[4].attributeNameValue = this.selectBegin.split(
        ","
      )[0];
      //给结束构件赋值
