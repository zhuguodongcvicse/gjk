<template>
  ...
  >
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="goStorageOfInf('form')">入库</el-button>
    </span>
  </el-dialog>
</template>
...
<script>
    ...
    import {updateInf} from "@/api/libs/hardwarelibinf";

    export default {
        //注入依赖，调用this.reload();用于刷新页面
        inject: ["reload"],
        //import引入的组件需要注入到对象中才能使用
        props: ["dialog", "approveUsers", "infWillToStorage", "allInfs"],
        components: {},
        //监听属性 类似于data概念
        computed: {
            ...mapGetters(["userInfo"])
                    ...
                    if (valid) {
                        ...
                        if (this.infWillToStorage.applyState === "0" || this.infWillToStorage.applyState == null) {
                            ...
                            saveApproval(approval).then(Response => {
                                let infTemp = {};
                                infTemp.id = this.infWillToStorage.id;
                                infTemp.applyState = "1";
                                infTemp.applyDesc = "已提交申请，请等待库管理员审批";
                                //修改构件审批状态成已提交申请
