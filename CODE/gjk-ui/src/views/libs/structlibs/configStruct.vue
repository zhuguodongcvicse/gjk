...
<template>
  ...
  >
    ...
    <div class="grid-content bg-purple-light_25s" >
      <el-button-group>
        <el-button type="primary" icon="el-icon-circle-plus-outline" @click="addNode" size="small">增加节点</el-button>
      ...
      >
        <el-table-column prop="fparamName" label="名称" type="scoped solt">
          <template slot-scope="{row}" v-if="row.delFlag!='1'">
            <el-input :disabled="mappedModel.dbId==row.parentId?false:true" v-model="row.fparamName"
                      size="mini"></el-input>
          ...
          <template slot-scope="{row}" v-if="row.delFlag!='1'">
            ...
            >
              <el-option
                ...
                <span style="float: right; color: #8492a6; font-size: 13px" v-if="item.version!='新'">v{{ item.version }}.0</span>
                <span style="float: right; color: #8492a6; font-size: 13px"v-else>{{ item.version }} </span>
              </el-option>
  ...
  export default {
    ...
    watch: {
      ...
      tmpModelSel: function (param) {
        ...
        if (vFlag && !this.isUpdate) {
          // this.mappedModel.id=Math.floor(Math.random() * 899999 + 100000)
          // this.mappedModel.dbId=Math.floor(Math.random() * 899999 + 100000)
          // this.mappedModel.fparamType=param
          // this.mappedModel.fparamName=""
          // this.mappedModel.children=[]
          this.mappedModel = {
            id: this.get_uuid(),
            dbId: this.get_uuid(),
            parentId: "0",
          ...
          // dbId: Math.floor(Math.random() * 899999 + 100000),
          // parentId: "0",
          // childrenIds: "",
          // fparamType: param,
          // structClassify: ""
    ...
    methods: {
      getPram(row) {
        ...
        if (row != null) {
          ...
          if (row.storageFlag == '1') {
            var sjId = this.get_uuid()
            this.mappedModel.dbId = sjId
            for (var k = 0; k < row.children.length; k++) {
              row.children[k].parentId = sjId
              row.children[k].id = this.get_uuid()
              row.children[k].dbId = this.get_uuid()
            }
          }
      ...
      delNode() {
        ...
        // console.log("得到fTableCurrentRow:" , this.fTableCurrentRow)
          ...
          getStructByFile({path: this.filePath}).then(res => {
            ...
            for (var prop in proMaps) {
              if (proMaps.hasOwnProperty(prop)) {
                ...
                let val = {dataType: prop, version: '新', id: this.get_uuid()}
