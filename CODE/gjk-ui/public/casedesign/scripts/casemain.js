...
function ondropLoadJSON(evt, graph, center, options) {
  ...
  //给芯片和接口的唯一标识拼接板卡的唯一标识  把板卡上各芯片的接口添加到数组
  if (result[0].properties.InternalLink != null) {
    for (const i in result[0].properties.InternalLink) {
      if (result[0].properties.InternalLink[i][0].uniqueId.indexOf(uuidRandom) === -1) {
        result[0].properties.InternalLink[i][0].uniqueId = uuidRandom + '_' + result[0].properties.InternalLink[i][0].uniqueId
      }
      ...
      if (result[0].properties.InternalLink[i][2].uniqueId.indexOf(uuidRandom) === -1) {
        result[0].properties.InternalLink[i][2].uniqueId = uuidRandom + '_' + result[0].properties.InternalLink[i][2].uniqueId
      }
      ...
      if (selection[i].properties.type === "card") {
        Q.confirm("是否 确认删除", function () {
          ...
          if (selection[0].properties.boardType === '0' || selection[0].properties.boardType === '1') {
            //移除fBoardList数组中的板卡
            for (let i = 0; i < fBoardList.length; i++) {
            ...
            for (let i = 0; i < selection[0].properties.chipList; i++) {
              for (let j = 0; j < selection[0].properties.chipList[i].infOfChipList; j++) {
                for (let k = 0; k < allInfOfFrontBoard; k++) {
