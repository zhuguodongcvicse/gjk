...
function ondropLoadJSON(evt, graph, center, options) {
	...
	var jsonObj = JSON.parse(options.json)
	...
	for (const i in result[0].properties.chipList) {
		...
		//芯片名称重命名
		/* if (result[0].properties.chipList[i].chipName.indexOf('+') != -1) {
			result[0].properties.chipList[i].chipName = result[0].properties.chipList[i].chipName.slice(0, result[0].properties.chipList[i].chipName.indexOf('+')) + '+' + result[0].properties.chipList[i].uniqueId
		} else {
			result[0].properties.chipList[i].chipName = result[0].properties.chipList[i].chipName + '+' + result[0].properties.chipList[i].uniqueId
		} */
		for (const j in result[0].properties.chipList[i].infOfChipList) {
			...
			//接口名称重命名
			/* if (result[0].properties.chipList[i].infOfChipList[j].infName.indexOf('+') != -1) {
				result[0].properties.chipList[i].infOfChipList[j].infName = result[0].properties.chipList[i].infOfChipList[j].infName.slice(0, result[0].properties.chipList[i].infOfChipList[j].infName.indexOf('+')) + '+' + result[0].properties.chipList[i].infOfChipList[j].uniqueId
			} else {
				result[0].properties.chipList[i].infOfChipList[j].infName = result[0].properties.chipList[i].infOfChipList[j].infName + '+' + result[0].properties.chipList[i].infOfChipList[j].uniqueId
			} */
			//把接口添加到数组
...
function initEditor(editor) {
	...
	button.onclick = function (evt) {
		if (graph.name == '正面机箱') {
			...
			bJson = graph.toJSON()
			//连线关系
			for (var i in bJson.datas) {
				if (bJson.datas[i].json.properties == null || bJson.datas[i].json.properties == "") {
					for (var j in bJson.datas) {
						if (bJson.datas[j].json.properties != null && bJson.datas[j].json.properties != "" &&
							bJson.datas[j].json.properties.infType != null && bJson.datas[j].json.properties.infType != "") {
							if (bJson.datas[i].json.from._ref == bJson.datas[j]._refId) {

								linkRelation.push(bJson.datas[j].json.properties)
							}
							if (bJson.datas[i].json.to._ref == bJson.datas[j]._refId) {
								linkRelation.push(bJson.datas[j].json.properties)
							}
						}
					}
			...
			var count = 0
			for (var i = 0; i < linkRelation.length;) {
				i = i + 2
				linkMap.set(linkRelation[count], linkRelation[++count])
				count++
			}
			var linkArr = Array.from(linkMap)
			/* for (var i in linkArr) {
				console.log("linkMap",linkArr[i])
			} */
				...
				if (fJson.datas[i].json.properties != null) {
					for (const j in fJson.datas) {
						if (fJson.datas[j].json.properties != null && fJson.datas[j].json.properties.boardType != null && fJson.datas[j].json.host._ref == fJson.datas[i]._refId) {
							// console.log("+++++++",fJson.datas[j].json.properties)
							// console.log("-----",fJson.datas[i])
				...
				if (bJson.datas[i].json.properties != null) {
					for (const j in bJson.datas) {
						if (bJson.datas[j].json.properties != null && bJson.datas[j].json.properties.boardType != null && bJson.datas[j].json.host._ref == bJson.datas[i]._refId) {
							// console.log("+++++++",bJson.datas[j].json.properties)
							// console.log("-----",bJson.datas[i])
			...
			fJsonStr = JSON.stringify(fJson)
			bJsonStr = JSON.stringify(bJson)
			postMessageParentData.cmd = "submitCaseJSON";
			postMessageParentData.params = [fJsonStr, bJsonStr, linkStr, boardNum];
			...
			for (var i in bJson.datas) {
				if (bJson.datas[i].json.properties == null || bJson.datas[i].json.properties == "") {
					for (var j in bJson.datas) {
						if (bJson.datas[j].json.properties != null && bJson.datas[j].json.properties != "" &&
							bJson.datas[j].json.properties.infType != null && bJson.datas[j].json.properties.infType != "") {
							if (bJson.datas[i].json.from != null && bJson.datas[j]._refId != null && bJson.datas[i].json.from._ref == bJson.datas[j]._refId) {

								linkRelation.push(bJson.datas[j].json.properties)
							}
							if (bJson.datas[i].json.to != null && bJson.datas[j]._refId != null && bJson.datas[i].json.to._ref == bJson.datas[j]._refId) {
								linkRelation.push(bJson.datas[j].json.properties)
							}
						}
					}
		...
		//outLineArr = Array.from(outLineMap)
		console.log("fJson", fJson)
		console.log("bJson", bJson)
	...
	function initInteraction() {
		...
		graph.interactionDispatcher.addListener(function (evt) {
			...
			if (evt.kind == Q.InteractionEvent.ELEMENT_CREATED && evt.data instanceof Q.Edge) {
				...
				//console.log('node.hasEdge()',node.hasEdge())
				/* var edge = evt.data;
				var edgeBundle = edge.getEdgeBundle();
				console.log('edgeBundle',edgeBundle)
				if(edgeBundle.length > 1){
					graph.removeElement(edge);
				} */
			}
