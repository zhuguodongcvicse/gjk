		...
		if (graph.name == '正面机箱') {
			...
			for (const i in clickBoardList) {
				for (const j in fBoardList) {
					if (fBoardList[j].uniqueId != clickBoardList[i].uniqueId) {
						clickBoardList.push(fBoardList[j])
						removeByValue(fBoardList, fBoardList[j])
					}
				}
			}
			fJson.datas[0].json.properties.frontBoardList = clickBoardList
			...
			clickBoardList = clickBoardList.concat(fBoardList)
