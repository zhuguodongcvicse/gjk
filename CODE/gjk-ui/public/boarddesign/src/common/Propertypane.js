		...
		createItemGroup: function (name, properties, boardType) {
			...
			if (boardType == 0 || boardType == 'calculateBoard') {
				...
				for (var i = 0; i < showExternalInf.length; i++) {
					//var j=i+1;
					var item1 = '<tr><td>' + showExternalInf[i].infName + '</td><td>' + linkTypeList[i] + '</td><td>' + ioTypeList[i] + '</td><td><button type="button" class="btn btn-default" data-toggle="modal" onclick="deletecalculateBoardLink(this);">删除</button></td></tr>';
					item0 = item0 + item1;
				}
