! function(Q) {
	...
	PropertyPane.prototype = {
		 ...
		 * @param properties
		 */
		createItemGroup: function(name, properties, clickData) {
			...
			/* if (clickData.infName != null) {
				...
				Q.createElement({
					tagName: 'select1',
					parent: group,
					html: '<div class="chipindex_modal_14s"><select id="selectInfIoType" class="chipindex_modal_14s"><option>2</option><option>0</option><option>1</option></select></div>'
					// html: '<select @change="" class="select" id=""><option>0</option><option>1</option><option>2</option></select></<select>'
				});
			} */ //style="width:30%"<label for="name">选择</lable>
		},
		...
		getCustomPropertyDefinitions: function(data) {
			...
			// console.log("data.propertyDefinitions",data.propertyDefinitions)
			...
			set: function(datas) {
				...
				if(datas.length == 1) {
					...
					for(var groupName in group) {
						this.createItemGroup(groupName, group[groupName], datas[0].properties);
					}
				}
			}
		}
