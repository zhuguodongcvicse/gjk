		...
		callback: function (editor) {
			...
			button.onclick = function () {
				...
				for (var i = 0; i < json.datas.length; i++) {
					//console.log("json.datas[i].json.properties.id",json.datas[i].json.properties.id)
					if(json.datas[i].json.properties.id != null && json.datas[i].json.properties.id != ""){
						infList.push(json.datas[i].json.properties.id)
						//console.log("infList",infList)
					}
				}
				json.datas[0].json.properties.infList = infList
				console.log("json", json)
			...
			var node = graph.createNode("cpu");
			node.image = 'images/cpu.svg';
			node.size = {
				width: 500
			};
				...
				graph.onclick = function (evt) {
					...
					for (let index in infArr) {
						...
						if (inf_uuid == infArr[index].id) {
							...
							var infRate = infArr[index].infRate
