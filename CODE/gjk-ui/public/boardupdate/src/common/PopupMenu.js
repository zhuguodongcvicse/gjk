      ...
      items.push({
        text: '删除', action: function (evt, item) {
          if (data.image == "images/Chip.svg" || data.image == "images/OpticalFiberMouth.svg"
            ...
            //删除
            // console.log("data.properties", JSON.stringify(data.properties))
            for (const i in chipList) {
              if (chipList[i].uniqueId.indexOf(data.properties.uniqueId) !== -1) {
            ...
            if (boardArr.boardType == 1) {
              ...
              for (let i = 0; i < showStartCpuList.length;) {
                if (showStartCpuList[i].uniqueId.indexOf(data.properties.uniqueId) !== -1) {
                  ...
                  i = 0
                } else if (showEndCpuList[i].uniqueId.indexOf(data.properties.uniqueId) !== -1) {
                  for (const j in dragCpuList) {
            ...
            if (boardArr.boardType === '2' || boardArr.boardType === '3') {
              for (const i in backBoardInfList) {
                if (backBoardInfList[i].uniqueId === data.properties.uniqueId) {
                  removeByValue(backBoardInfList, backBoardInfList[i])
                }
              }
            }
            for (const i in dragCpuList) {
              if (dragCpuList[i].uniqueId.indexOf(data.properties.uniqueId) !== -1) {
