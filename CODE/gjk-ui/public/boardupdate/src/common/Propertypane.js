! function (Q) {
	...
	PropertyPane.prototype = {
		...
		createItemGroup: function (name, properties, boardType) {
			...
			//calculateBoard板卡，交换芯片配置
			/* if(properties["C:boardname"].name == "boardname"){
				console.log("properties",key)
			} */
			if (boardType == 0 || boardType == 'calculateBoard') {
				Q.createElement({
					tagName: 'h4',
					parent: group,
					html: '交换芯片'
				...
				var item0 = '';
				//console.log("showExternalInf1",showExternalInf)
				for (var i = 0; i < showExternalInf.length; i++) {
					var showLinkTypeDict
					for (const j in calculateBoardLinkType) {
						if (linkTypeList[i] == calculateBoardLinkType[j].value) {
							showLinkTypeDict = calculateBoardLinkType[j].label
						}
					}
					for (const j in calculateBoardIoType) {
						if (ioTypeList[i] == calculateBoardIoType[j].value) {
							showIoTypeDict = calculateBoardIoType[j].label
						}
					}
					var item1 = '<tr><td>' + showExternalInf[i].infName + '</td><td>' + showLinkTypeDict + '</td><td>' + showIoTypeDict + '</td><td><button type="button" class="btn btn-default" data-toggle="modal" onclick="deletecalculateBoardLink(this);">删除</button></td></tr>';
					item0 = item0 + item1;
				}
				//console.log("showExternalInf2",showExternalInf)
				...
				var html1 = '<div class="propertypane_table_14s"><table class="table table-bordered table_14s"><tr><th>inf Name</th><th>linkType</th><th>ioType</th><th>操作</th></tr><tbody class="tbody1_14s" id="tbody1"> ' + item0 + '</tbody></table></div>';

				Q.createElement({
					tagName: 'button1',
					parent: group,
					html: '<button type="button" class="btn btn-default propertypane_tablebtn_14s" data-toggle="modal" data-target="#myModal" onclick="loadcalculateBoardDatas();">新增</button>'
				});
			...
			//FpgaBoard板卡  内部互联配置
			if (boardType == 1 || boardType == 'FpgaBoard') {
				Q.createElement({
					tagName: 'h4',
					parent: group,
					html: '内部互联'
				...
				//表格内容 
				//var arr0=['输入','双路','输入','双路','输入',];
				// console.log("showStartCpuList",showStartCpuList)
				// console.log("showStartInfList",showStartInfList)
				// console.log("showEndCpuList",showEndCpuList)
				// console.log("showEndInfList",showEndInfList)
				var item3 = '';
				for (var i = 0; i < showStartInfList.length; i++) {
					//var j=i+1;
					var item4 = '<tr><td>' + showStartCpuList[i].ID + '</td><td>' + showStartInfList[i].ID + '</td><td>' + showEndCpuList[i].ID + '</td><td>' + showEndInfList[i].ID + '</td><td><button type="button" class="btn btn-default" data-toggle="modal" onclick="deleteInternalLink(this);">删除</button></td></tr>';
					item3 = item3 + item4;
				}
				//var html3='<br/><div style="height:200px;overflow-y:auto;"><table class="table table-bordered"><tr><th>接口名称</th><th>接口类型</th></tr><tbody> <tr> <td>接口1</td> <td>输入</td> </tr> <tr> <td>接口2</td> <td>双路</td> </tr> <tr><td>接口2</td> <td>双路</td></tr><tr><td>接口3</td> <td>双路</td></tr><tr><td>接口4</td> <td>双路</td></tr></tbody></table></div>';
				var html2 = '<div class="propertypane_table_14s"><table class="table table-bordered table_14s"><tr><th>startCpu</th><th>startInf</th><th>endCpu</th><th>endInf</th><th>操作</th></tr><tbody class="tbody1_14s" id="tbody2"> ' + item3 + '</tbody></table></div>';

				Q.createElement({
					tagName: 'button1',
					parent: group,
					html: '<button type="button" class="btn btn-default propertypane_tablebtn_14s" data-toggle="modal" data-target="#myModal2" onclick="loadFpgaBoardDatas();">新增</button>'
				});
				Q.createElement({
					tagName: 'table1',
					parent: group,
						...
						switch (selectStartCpuValue) {
							case dragCpuList[i].ID.toString():
								if (dragCpuList[i].infOfChipList.length != 0) {
									dragCpuList[i].infOfChipList = dragCpuList[i].infOfChipList
									$("#selectStartInf").empty();
									// console.log("dragCpuList[i].infOfChipList", dragCpuList[i].infOfChipList)
				...
				var selectEndCpu = document.getElementById("selectEndCpu");
				selectEndCpu.onchange = function () {
					selectEndCpuValue = selectEndCpu.value;
					for (const i in dragCpuList) {
						switch (selectEndCpuValue) {
							case dragCpuList[i].ID.toString():
								if (dragCpuList[i].infOfChipList.length != 0) {
									...
									// console.log("selectEndInfHtml***",selectEndInfHtml)
			...
			set: function (datas) {
				...
				if (datas.length == 1) {
					...
					for (var groupName in group) {
						this.createItemGroup(groupName, group[groupName], datas[0].properties.boardType);
					}
				}
			}
		}
