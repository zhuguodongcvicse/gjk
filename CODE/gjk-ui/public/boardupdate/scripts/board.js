	...
	switch (event.data.cmd) {
		case 'boardupdate':
			...
			if (boardArr.boardType == 0) {
				...
				for (const j in alreadyExistExchangeLink) {
					for (const i in allInfList) {
						if (allInfList[i].id == alreadyExistExchangeLink[j][0].id) {
							//删除已经添加的ExchangeCpu上的接口
							allInfList[i].ifShowInSelect = 1
							// allInfList = deepClone(allInfList)
						}/*  else {
							allInfList[i].ifShowInSelect = 0
						} */
					}
				}
				// console.log("allInfList", allInfList)
